...
/*! accel streaming read data status */
int16_t accel_rslt = COINES_SUCCESS;

/*! gyro streaming read data status */
int16_t gyro_rslt = COINES_SUCCESS;

/*! accel valid sample count */
uint32_t accel_valid_sample_count;

/*! gyro valid sample count */
uint32_t gyro_valid_sample_count;
...
/*! This API is used to print the read sensor data */
void print_accel_gyro_data(void);
...
void read_sensor_data(void)
{
    ...
    accel_rslt = coines_read_stream_sensor_data(1, NO_OF_SAMPLES, &bmi08x_accel_stream_buffer[0], &accel_valid_sample_count);

    memset(&bmi08x_gyro_stream_buffer[0], 0, COINES_STREAM_RSP_BUF_SIZE);
    gyro_rslt = coines_read_stream_sensor_data(2, NO_OF_SAMPLES, &bmi08x_gyro_stream_buffer[0], &gyro_valid_sample_count);
...
/*!
 *  @brief This API is used to print read sensor data
 *
 *  @param[in] void
 *
 *  @return void
...
void print_accel_gyro_data(void)
{
    ...
    if (accel_rslt == COINES_SUCCESS)
    {
        ...
        for (idx = 0; idx < accel_valid_sample_count; idx++)
        {
    ...
    if (gyro_rslt == COINES_SUCCESS)
    {
        ...
        for (idx = 0; idx < gyro_valid_sample_count; idx++)
        {
