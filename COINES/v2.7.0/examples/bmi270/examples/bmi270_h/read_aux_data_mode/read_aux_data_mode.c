 ...
 *  @return None
 */
static void bmi2_aux_conf(uint16_t mag_odr, struct bmm150_dev *bmm, struct bmi2_dev *bmi);
    ...
    if (bmm.chip_id == BMM150_CHIP_ID)
    {
        ...
        while (1)
        {
                ...
                (sens_data.status & BMI2_DRDY_GYR))
            {
                ...
                acc_x = lsb_to_mps2(sens_data.acc.x, 2, bmi.resolution);
                acc_y = lsb_to_mps2(sens_data.acc.y, 2, bmi.resolution);
                acc_z = lsb_to_mps2(sens_data.acc.z, 2, bmi.resolution);

                /* Converting lsb to degree per second for 16 bit gyro at 2000dps range. */
                gyr_x = lsb_to_dps(sens_data.gyr.x, 2000, bmi.resolution);
                gyr_y = lsb_to_dps(sens_data.gyr.y, 2000, bmi.resolution);
                gyr_z = lsb_to_dps(sens_data.gyr.z, 2000, bmi.resolution);
...
static int8_t bmi2_aux_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    rslt = bmi2_read_aux_man_mode(reg_addr, reg_data, length, dev);
...
static int8_t bmi2_aux_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    rslt = bmi2_write_aux_man_mode(reg_addr, reg_data, length, dev);
