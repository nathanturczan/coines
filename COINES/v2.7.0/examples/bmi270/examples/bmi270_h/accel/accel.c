 ...
 */
static int8_t set_accel_config(uint8_t range, struct bmi2_dev *bmi);
...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = set_accel_config(range, &bmi);
        bmi2_error_codes_print_result(rslt);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            while (indx <= limit)
            {
                ...
                if ((rslt == BMI2_OK) && (sens_data.status & BMI2_DRDY_ACC))
                {
                    ...
                    x = lsb_to_mps2(sens_data.acc.x, 2, bmi.resolution);
                    y = lsb_to_mps2(sens_data.acc.y, 2, bmi.resolution);
                    z = lsb_to_mps2(sens_data.acc.z, 2, bmi.resolution);
