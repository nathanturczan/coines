 ...
 *
 *  @param[out] range    : Range of Accel
 *  @param[in] bmi       : Structure instance of bmi2_dev.
 *
 *  @return Status of execution.
 */
static int8_t set_accel_gyro_config(uint8_t range, struct bmi2_dev *bmi);
...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = set_accel_gyro_config(range, &bmi);
        bmi2_error_codes_print_result(rslt);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            while (indx <= limit)
            {
                ...
                if ((rslt == BMI2_OK) && (sensor_data.status & BMI2_DRDY_ACC) && (sensor_data.status & BMI2_DRDY_GYR))
                {
                    ...
                    acc_x = lsb_to_mps2(sensor_data.acc.x, 2, bmi.resolution);
                    acc_y = lsb_to_mps2(sensor_data.acc.y, 2, bmi.resolution);
                    acc_z = lsb_to_mps2(sensor_data.acc.z, 2, bmi.resolution);

                    /* Converting lsb to degree per second for 16 bit gyro at 2000dps range. */
                    gyr_x = lsb_to_dps(sensor_data.gyr.x, 2000, bmi.resolution);
                    gyr_y = lsb_to_dps(sensor_data.gyr.y, 2000, bmi.resolution);
                    gyr_z = lsb_to_dps(sensor_data.gyr.z, 2000, bmi.resolution);
...
static int8_t set_accel_gyro_config(uint8_t range, struct bmi2_dev *bmi)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        range = config[ACCEL].cfg.acc.range;
