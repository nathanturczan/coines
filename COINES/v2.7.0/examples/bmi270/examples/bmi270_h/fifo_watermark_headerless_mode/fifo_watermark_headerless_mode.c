...
#define BMI270_H_FIFO_RAW_DATA_BUFFER_SIZE  UINT16_C(800)

/*! Length of data to be read from FIFO */
#define BMI270_H_FIFO_RAW_DATA_USER_LENGTH  UINT16_C(800)
 ...
 */
#define BMI270_H_FIFO_ACCEL_FRAME_COUNT     UINT8_C(55)

/*! Number of gyro frames to be extracted from FIFO */
#define BMI270_H_FIFO_GYRO_FRAME_COUNT      UINT8_C(55)

/*! Setting a watermark level in FIFO */
#define BMI270_H_FIFO_WATERMARK_LEVEL       UINT16_C(650)
...
int main(void)
{
    ...
    uint16_t accel_frame_length = BMI270_H_FIFO_ACCEL_FRAME_COUNT;

    uint16_t gyro_frame_length = BMI270_H_FIFO_GYRO_FRAME_COUNT;
     ...
     */
    uint8_t fifo_data[BMI270_H_FIFO_RAW_DATA_BUFFER_SIZE] = { 0 };
    ...
    struct bmi2_sens_axes_data fifo_accel_data[BMI270_H_FIFO_ACCEL_FRAME_COUNT] = { { 0 } };

    /* Array of gyro frames -> Total bytes =
     * 50 * (6 axes bytes) = 300 bytes */
    struct bmi2_sens_axes_data fifo_gyro_data[BMI270_H_FIFO_GYRO_FRAME_COUNT] = { { 0 } };
    ...
    rslt = bmi270_h_sensor_enable(sensor_sel, 2, &dev);
    bmi2_error_codes_print_result(rslt);
    ...
    fifoframe.length = BMI270_H_FIFO_RAW_DATA_USER_LENGTH;
    ...
    fifoframe.wm_lvl = BMI270_H_FIFO_WATERMARK_LEVEL;
    ...
    while (try <= 10)
    {
        ...
        if ((rslt == BMI2_OK) && (int_status & BMI2_FWM_INT_STATUS_MASK))
        {
            ...
            accel_frame_length = BMI270_H_FIFO_ACCEL_FRAME_COUNT;
            gyro_frame_length = BMI270_H_FIFO_GYRO_FRAME_COUNT;
...
static int8_t set_accel_gyro_config(struct bmi2_dev *dev)
{
    ...
    rslt = bmi270_h_get_sensor_config(config, 2, dev);
    bmi2_error_codes_print_result(rslt);
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = bmi270_h_set_sensor_config(config, 2, dev);
        bmi2_error_codes_print_result(rslt);
    }
