...
int main(void)
{
    ...
    /* Initialize bmi270_h. */
    rslt = bmi270_h_init(&bmi2_dev);
    bmi2_error_codes_print_result(rslt);

    if (rslt == BMI2_OK)
    {
        /* Enable the selected sensor. */
        rslt = bmi270_h_sensor_enable(sensor_sel, 2, &bmi2_dev);
        bmi2_error_codes_print_result(rslt);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            if (rslt == BMI2_OK)
            {
                ...
                rslt = bmi270_h_map_feat_int(&sens_int, 1, &bmi2_dev);
                bmi2_error_codes_print_result(rslt);
...
static int8_t set_feature_config(struct bmi2_dev *bmi2_dev)
{
    ...
    rslt = bmi270_h_get_sensor_config(&config, 1, bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = bmi270_h_set_sensor_config(&config, 1, bmi2_dev);
        bmi2_error_codes_print_result(rslt);
    }
