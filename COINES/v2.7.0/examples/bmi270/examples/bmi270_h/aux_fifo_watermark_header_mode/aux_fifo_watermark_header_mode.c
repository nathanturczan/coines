 ...
 * aux_frame_len = 8, header_byte = 1.
 * fifo_accel_frame_count = (650 / (6 + 6 + 8 + 1 )) = 30 frames
 * NOTE: Extra frames are read in order to get sensor time
 */
#define BMI2_FIFO_ACCEL_FRAME_COUNT     UINT8_C(50)
...
#define BMI2_FIFO_AUX_FRAME_COUNT       UINT8_C(50)
...
int main(void)
{
    ...
    /* As per datasheet, aux interface with bmi270_h will support only for I2C */
    ...
    /* Initialize bmi270_h. */
    rslt = bmi270_h_init(&aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi270_h_get_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
    ...
    rslt = bmi270_h_set_sensor_config(config, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);

    /* NOTE:
     * Accel and gyro enable must be done after setting configurations
     */
    rslt = bmi270_h_sensor_enable(sensor_list, 3, &aux_bmi2_dev);
    bmi2_error_codes_print_result(rslt);
