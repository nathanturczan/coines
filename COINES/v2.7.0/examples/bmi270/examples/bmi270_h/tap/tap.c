...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        rslt = bmi270_h_sensor_enable(sens_list, 4, &dev);
        bmi2_error_codes_print_result(rslt);
        ...
        if (rslt == BMI2_OK)
        {
            ...
            rslt = bmi270_h_get_sensor_config(&config, 1, &dev);
            bmi2_error_codes_print_result(rslt);

            if (rslt == BMI2_OK)
            {
                /* Map the tap feature interrupt. */
                rslt = bmi270_h_map_feat_int(&sens_int, 1, &dev);
                bmi2_error_codes_print_result(rslt);
                ...
                do
                {
                    ...
                    if (int_status & BMI270_H_TAP_STATUS_MASK)
                    {
                        printf("Tap interrupt is generated\n");
                        rslt = bmi2_get_regs(BMI270_H_TAP_STATUS_REG, &data, 1, &dev);
                        bmi2_error_codes_print_result(rslt);

                        if (data & BMI270_H_SINGLE_TAP_MASK)
                        {
                            printf("Single tap asserted\n");
                        }

                        if (data & BMI270_H_DOUBLE_TAP_MASK)
                        {
                            printf("Double tap asserted\n");
                        }

                        if (data & BMI270_H_TRIPLE_TAP_MASK)
                        {
