...
int main(void)
{
     ...
     */
    rslt = bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_ANY_MOT_INT, BMA4_ENABLE, &bma);
    bma4_error_codes_print_result("bma423_map_interrupt status", rslt);

    bma.delay_us(2000, bma.intf_ptr);

    rslt = bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_NO_MOT_INT, BMA4_ENABLE, &bma);
    bma4_error_codes_print_result("bma423_map_interrupt status", rslt);
