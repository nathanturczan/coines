 ...
 */
int8_t bme680_i2c_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t count)
{
    return coines_read_i2c(COINES_I2C_BUS_0, dev_addr, reg_addr, reg_data, count);
}

/*!
 * I2C write function map to COINES platform
 */
int8_t bme680_i2c_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t count)
{
    return coines_write_i2c(COINES_I2C_BUS_0, dev_addr, reg_addr, reg_data, count);
}

/*!
 * SPI read function map to COINES platform
 */
int8_t bme680_spi_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t count)
{
    return coines_read_spi(COINES_SPI_BUS_0, dev_addr, reg_addr, reg_data, count);
}

/*!
 * SPI write function map to COINES platform
 */
int8_t bme680_spi_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t count)
{
    return coines_write_spi(COINES_SPI_BUS_0, dev_addr, reg_addr, reg_data, count);
}

static void hardware_init(void)
{
    struct coines_board_info board_info;
    if (coines_open_comm_intf(COINES_COMM_INTF_USB, NULL) < 0)
    {
    ...
    if (coines_get_board_info(&board_info) == COINES_SUCCESS)
    {
    ...
    bme680_g.read = bme680_i2c_read;
    bme680_g.write = bme680_i2c_write;
    bme680_g.delay_ms = coines_delay_msec;
    ...
    bme680_g.read = bme680_spi_read;
    bme680_g.write = bme68_spi_write;
    bme680_g.delay_ms = coines_delay_msec;
                    ...
                    state_save_fct state_save, uint32_t save_intvl)
{
    ...
    while (1)
    {
        ...
        {
            printf("bsec_sensor_control Lib function error %d \n", bsec_status);
        }
