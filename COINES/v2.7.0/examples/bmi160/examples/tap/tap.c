...
/* functions */
/*********************************************************************/
int8_t bmi160_i2c_read(uint8_t dev_addr,uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    return coines_read_i2c(COINES_I2C_BUS_0, dev_addr, reg_addr, reg_data,length);
}

/*!
 * I2C write function map to COINES platform
 */
int8_t bmi160_i2c_write(uint8_t dev_addr,uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    return coines_write_i2c(COINES_I2C_BUS_0, dev_addr, reg_addr, reg_data, length);
}

/*!
 * SPI read function map to COINES platform
 */
int8_t bmi160_spi_read(uint8_t dev_addr,uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    return coines_read_spi(COINES_SPI_BUS_0, dev_addr, reg_addr, reg_data, length);
}

/*!
 * SPI write function map to COINES platform
 */
int8_t bmi160_spi_write(uint8_t dev_addr,uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    return coines_write_spi(COINES_SPI_BUS_0, dev_addr, reg_addr, reg_data, length);
}
...
static void init_bmi160_sensor_driver_interface(void)
{
    ...
    bmi160dev.write = bmi160_i2c_write;
    bmi160dev.read = bmi160_i2c_read;
    ...
    /* link read/write/delay function of host system to appropriate
     *  bmi160 function call prototypes */
    bmi160dev.write = bmi160_spi_write;
    bmi160dev.read = bmi160_spi_read;
    bmi160dev.delay_ms = coines_delay_msec;
    ...
    if (rslt == BMI160_OK)
    {
        ...
        while (loop < 10)
        {
            ...
            if (rslt == BMI160_OK)
            {
                ...
                {
                    printf("Single tap, iter:%d, time:%ld ms, delta:%ld ms, int_status:0x%x\n",
                           loop++,
                           current_time,
                ...
                {
                    printf("Double tap, iter:%d, time:%ld ms, delta:%ld ms, int_status:0x%x\n",
                           loop++,
                           current_time,
                           current_time - last_time,
                           int_status.data[0]);
